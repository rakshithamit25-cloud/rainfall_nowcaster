...
def get_place(lat, lon):
    # Open-Meteo reverse geocoding is not available as a coordinate endpoint,
    # so show coordinates if a place lookup is unavailable.
...
for c,(label,value) in zip(cols,items):
    with c:
        st.markdown(
            f'<div class="card"><div class="small">{label}</div><div class="big">{value}</div></div>',
            unsafe_allow_html=True
        )
